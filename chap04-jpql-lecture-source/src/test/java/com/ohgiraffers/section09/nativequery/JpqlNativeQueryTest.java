...
public class JpqlNativeQueryTest {
    ...
    * - 복잡한 쿼리, 특정 데이터베이스 에서만 사용가능한 기능을 사용할떄 사용한다.
    *
    * 네이티브 쿼리 API 종류 3가지
    * 1. 결과 타입 정의 (엔티티클래스만 지정 가능)
    *   public Query createNativeQuery(String sqlString, class resultQuery)
    * 2. 결과 타입을 정의할 수 없을 떄
    ...
    * */
    ...
    @Test
    @DisplayName("결과타입 지정해서 Native Query 사용하기")
    ...
    void test4() {
        ...
        Query query = entityManager.createNamedQuery("section09.Menu.findMenuAndCategory")
                .setParameter(1, menuCode);
        MenuCategoryResonseDTO menu = (MenuCategoryResonseDTO) query.getSingleResult();
        System.out.println("menu = " + menu);
        // then
